...
def caption_image_beam_search(encoder, decoder, image_path, 
    ...
    while True:
        ...
        seqs_alpha = torch.cat([seqs_alpha, alpha[prev_word_inds].unsqueeze(1)],dim=1)  # (s, step+1, enc_image_size, enc_image_size)        
...
def visualize_att(image_path, seq, alphas,rev_word_map, smooth=True,model='model_1', image_set='untrained'):
    """
    ...
    for t in range(len(words)):
        ...
        if current_word not in ['<start>', '<end>']:
            image_info = image_path[image_path.find("test_")+len("test_"):]
            # which set and type (new or old) is the image
            image_type = image_info[:image_info.rfind('/')]
...
def eval_trials(trial, img_scores, img_caption, gt, 
    ...
    set_type = args.test_data_type    #New or old   
...
def evaluate_single_trial(word_target, word_map,target_prob, foil_prob):
...
def eval_trials_edit_distance(trial, img_caption, gt, model="model_1"):
    ...
    correct = res_df[res_df["correct"]==1].count()["target_image"]
    incorrect = res_df[res_df["correct"]==0].count()["target_image"]    
    print("{}'s accuracy for {} is {} edit".format(model, set_type, correct/total))
    return "{}, {}, {} \n".format(model, set_type, correct/total)
...
parser.add_argument('--test_data_type', '-st', help="Test data type ie. new or old")
parser.add_argument('--eval_metric', '-et', type=str, help="Evaluation metric to use ie. edit or prob")
parser.add_argument('--untrained_model_dir', '-m', help="Path to untrained models")
...
if __name__ == '__main__':
    ...
    for n in range(1, args.num_models+1):
        ...
        decoder = None
        encoder = None
        # Load untrained model from
        if args.untrained_model_dir:
            cur_model = "model_" + str(n)
            model_path = os.path.join(args.untrained_model_dir, os.path.join(cur_model, model_file))
            # Load model
            checkpoint = None
            try:
                checkpoint = torch.load(model_path, map_location='cpu')
            except:
                print("Issue with: " + cur_model)
                continue
            ...
            # When untrained models are not saved but initialized directly here
